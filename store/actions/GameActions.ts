...
const startHand = () => async (dispatch, getState) => {
    const { game: { deck,
        isLastOfDeck,
        current_hand: { phase } } }: RootState = getState()
...
const makeBet = (ammount: number) => (dispatch, getState) => {
    const { game: { current_hand: { phase } } }: RootState = getState()
...
const doInitialDeal = () => async (dispatch, getState) => {
    const { game: { deck, current_hand: { phase } } }: RootState = getState()
...
const playerDraw = (isDoubleDown = false) => async (dispatch, getState) => {
    const { game: { deck } }: RootState = getState()
    const { data: { cards, remaining } } = await drawCards(deck)
    const phase_base: GamePhase[] = [BankerAction, Endgame]
    dispatch({
...
export {
    startHand, makeBet, doInitialDeal, doSurrender,
